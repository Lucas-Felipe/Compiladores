...
Assign::Assign(Expression *i, Expression *e) : Statement(NodeType::ASSIGN), id(i), expr(e)
{
    ...
    if (id->type == ExprType::BOOL && expr->type == ExprType::INT) {
        expr->ConvertIntToBool();  // Converte antes de verificar os tipos
    }
...
bool Expression :: ConvertIntToBool() {
    if (this->type == ExprType::INT) {
        int value = std::stoi(this->token->lexeme);  // Converte o valor do token para um inteiro
        this->token->lexeme = value != 0 ? "true" : "false";  // Atualiza o lexeme com "true" ou "false"
        this->type = ExprType::BOOL;  // Atualiza o tipo da expressão
        return value != 0;  // Retorna o valor booleano
    }
    return false;  // Caso não seja um inteiro
}